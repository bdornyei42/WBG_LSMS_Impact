...
class OpenAlexFetcher:
    ...
    def search_family(self, family: dict,
        ...
        for term in terms:
            ...
            params_query: dict = {
                "search":   _build_search_query(term),
                "select":   _OA_SELECT,
                "per_page": PAGE_SIZE,
            }
            if since_date:
...
def deduplicate(papers: list[dict],
    ...
    # ── Multi-family relevance boost ─────────────────────────────────────────
    # A paper that independently matched 2+ DISTINCT survey families (each a
    # Tier-A full-text match) is almost certainly using LSMS microdata, even if
    # its abstract only says "nationally-representative survey data". Bump such
    # papers to at least score 2 so they are not stranded in the backup sheet.
    # Example: a cross-country paper naming the Malawi IHS, Tanzania NPS, and
    # Nigeria GHS-Panel surveys in its data section.
    for p in order:
...
_UNAMBIGUOUS_MARKERS = (
    "living standards", "socioeconomic survey", "socioeconomic panel",
    "national panel survey", "integrated household", "general household survey",
    "enqu", "integrated surveys on agriculture",
    "high-frequency phone survey", "phone survey",
)


# The only ' and ' constructions that are boolean operators in the original
# keyword list join a survey term to an LSMS-ISA country name.
# Everything else ("... Wave 4 and Wave 5", "... 2014 and 2016 data") is prose.
_AND_COUNTRIES = {
    "burkina faso", "mali", "nigeria", "niger",
    "ethiopia", "uganda", "malawi", "tanzania",
}


def _is_compound_and(term: str) -> bool:
    """
    True only when ' and ' joins a survey term to an LSMS-ISA country name.
    'HFPS and Burkina Faso'                        -> True  (boolean AND)
    'Ethiopia ... (ESPS) Wave 4 and Wave 5'        -> False (prose)
    'Ethiopia Socioeconomic Survey (ESS) 2012, 2014 and 2016 data' -> False
    """
    parts = re.split(r"\s+and\s+", term, flags=re.IGNORECASE)
    if len(parts) != 2:
        return False
    return parts[1].strip().lower() in _AND_COUNTRIES


def _build_search_query(term: str) -> str:
    """OpenAlex search= string. Compound AND -> boolean; otherwise phrase."""
    if _is_compound_and(term):
        parts = re.split(r"\s+and\s+", term, flags=re.IGNORECASE)
        return " AND ".join('"' + p.strip() + '"' for p in parts)
    return '"' + term + '"'


def _term_tier(term: str) -> str:
    """Classify a single (non-compound) term into tier A / B / C."""
    t = term.strip()
    n_words = len(t.split())

    if n_words == 1:
        letters = re.sub(r"[-\d()]", "", t)
        if letters and letters.isupper() and len(t) <= 6:
            return TIER_C
        return TIER_B

    low = t.lower()
    if n_words >= 3 or any(m in low for m in _UNAMBIGUOUS_MARKERS):
        return TIER_A
    return TIER_B
...
def _passes_filters(title: str, abstract: str, term: str,
                    context_hints: Optional[list],
                    primary_topic: Optional[dict]) -> tuple:
    """
    Master gate. Returns (passes: bool, tier: str, reason: str).
    Recall is favoured for unambiguous terms; precision for ambiguous acronyms.
    """
    if _is_compound_and(term):
        parts = re.split(r"\s+and\s+", term, flags=re.IGNORECASE)
        for p in parts:
            ok, _, why = _passes_filters(title, abstract, p.strip(),
                                         context_hints=None,
    ...
    if tier == TIER_B:
        if _word_present(term, text, case_sensitive=False):
            return True, TIER_B, "term in title/abstract"
        return False, TIER_B, "term not in title/abstract"
    ...
    if context_hints and not any(_word_present(h, text, case_sensitive=False)
        ...
        return False, TIER_C, "no country/context word near acronym"
    if not _is_relevant_topic(primary_topic):
        return False, TIER_C, "irrelevant discipline"
    return True, TIER_C, "acronym + context + relevant discipline"
...
def _detect_journal_tier(venue: str, pub_type: str) -> str:
    ...
    Rules (in order):
      ...
      3. Any remaining JOURNAL ARTICLE                                 -> Tier 4
         (peer-reviewed but unrecognised journals get the lowest tier,
          never left blank)
      4. Anything else with no venue                                   -> blank

    This guarantees: every peer-reviewed journal article receives a tier, and
    every non-journal output is labelled 'WP', eliminating the 'no marker' gap.
    """
    pt = pub_type or ""
...
Writes the Analysis sheet cell-by-cell, matching the exact styling from the
reference file (FF1F5C99 dark-blue headers, bold highlights, 75/48/23/23 col widths).
Charts anchored at column E (index 4), width=15, height=7.5.
"""
...
C_WHITE = "FFFFFF"
C_LBLUE = "C5D9F1"   # light row highlight
...
def write_analysis_sheet(ws, papers, current_fy, completed_fys,
    ...
    bar.title = "LSMS Papers per Fiscal Year (completed FYs only)"
    ...
    pie = PieChart()
    pie.title = "Journal Tier Distribution (analysed set)"
...
def export_excel(papers: list, review: list, search_log: list, output_path: str,
    ...
    Sheet order:
      ...
      2. Analysis                - metrics (left) + both charts (top-right)
    ...
    for f in _KW:
        ...
        for t in f["terms"]:
            tier = "AND" if _is_compound_and(t) else _term_tier(t)
            kw_rows.append((f["label"], f["region"], t, tier, _TIER_DESC[tier],
                            _build_search_query(t), hints if tier == "C" else ""))
...
def run_discovery(
    ...
    for f_idx, family in enumerate(families, 1):
        ...
        # OpenAlex batched OR query — prints its own live progress
    ...
    # Filter pre-LSMS-program papers (established 1980)
    n_before = len(all_papers)
    all_papers = [p for p in all_papers if (p.get("year") or 0) >= min_year]
    if verbose and len(all_papers) < n_before:
        print(f"  Dropped {n_before - len(all_papers)} papers published before {min_year}", flush=True)

    # ── RELEVANCE GATE ───────────────────────────────────────────────────────
    # Only papers scoring >= 2 enter the analysed set. Score 2 means at least one
    # of: survey name in the abstract, explicit data-use language, LSMS/World Bank
    # named, or strong empirical vocabulary. Score 3 adds survey name in the title.
    #
    # Scores 0 and 1 are set aside entirely:
    #   1 = matched only via OpenAlex full-text; no LSMS signal in title/abstract
    #   0 = review / meta-analysis detected
    # They are written to the 'Not Relevant (Backup)' sheet and are excluded from
    # every headline count, every chart, and the total paper count.
    ...
    clean, review = deduplicate(all_papers, existing_df, fuzzy_threshold)
